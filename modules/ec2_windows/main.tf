      ...
      },
      {
        ...
        inputs = {
          runCommand = [
            ...
            "$service = Get-Service -Name 'MT5Adapter' -ErrorAction SilentlyContinue",
            "if ($service) {",
            "    Stop-Service -Name 'MT5Adapter' -Force -ErrorAction SilentlyContinue",
            "    & 'C:\\ProgramData\\chocolatey\\bin\\nssm.exe' stop 'MT5Adapter' confirm 2>$null | Out-Null",
            "}",
            ...
            "} else {",
            "    Push-Location $RepoDir; git pull origin '{{ RepoBranch }}'; Pop-Location",
            "}",
      ...
      },
      {
        ...
        inputs = {
          runCommand = [
            ...
            "Write-Host 'MT5Adapter service configured and started successfully.'"
          ]
        }
      }
    ]
